...
// SetupLKEClusterSecret adds a controller that propagates LKECluster connection
// secrets to the connection secrets of their resource claims.
func SetupLKEClusterSecret(mgr ctrl.Manager, l logging.Logger) error {
	name := secret.ControllerName(v1alpha1.LKEClusterGroupKind)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithEventFilter(resource.NewPredicates(resource.AnyOf(
			resource.AllOf(resource.IsControlledByKind(computev1alpha1.KubernetesClusterGroupVersionKind), resource.IsPropagated()),
			resource.AllOf(resource.IsControlledByKind(workloadv1alpha1.KubernetesTargetGroupVersionKind), resource.IsPropagated()),
			resource.AllOf(resource.IsControlledByKind(v1alpha1.LKEClusterGroupVersionKind), resource.IsPropagator())))).
